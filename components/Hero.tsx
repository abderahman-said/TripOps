...
import { SLIDES } from "@/constants/landing";
...
export function Hero() {
  ...
  const places = ["شرم الشيخ...", "الغردقة...", "مكة المكرمة...", "أسوان...", "لوكسور..."];

  // Typewriter
  useEffect(() => {
    let idx = 0;
    const ph = places[phIdx];
    let forward = true;
    const t = setInterval(() => {
      if (forward) {
        idx++;
        setTyped(ph.slice(0, idx));
        if (idx >= ph.length) { forward = false; setTimeout(() => { forward = false; }, 1000); }
      } else {
        idx--;
        setTyped(ph.slice(0, idx));
        if (idx === 0) { setPhIdx(p => (p + 1) % places.length); clearInterval(t); }
      }
    }, 80);
  ...
  // Slide auto-advance
  const advance = useCallback(() => setCur(p => (p + 1) % SLIDES.length), []);
  useEffect(() => {
    ...
    <section className="relative md:h-screen min-h-[650px] md:min-h-[780px] overflow-hidden pb-20 md:pb-0">
      ...
      {SLIDES.map((sl, i) => (
        <div key={i} className="absolute inset-0 transition-opacity duration-1200"
          ...
          <div className="absolute inset-0" style={{ background: "linear-gradient(to top, rgba(8,26,75,.95) 0%, rgba(8,26,75,.45) 55%, rgba(8,26,75,.15) 100%)" }} />
      ...
      <div className="relative pt-[120px] md:pt-[150px] z-10 h-full flex flex-col items-center text-center px-4" dir="rtl">
        ...
        {/* Headline */}
        <h1 className="leading-none flex justify-center gap-2 mb-4" style={{ letterSpacing: "-0.025em" }}>
          {[s.line1, s.line2].map((word, wi) => (
            <div key={`${cur}-${wi}`} className="animate-floatUp overflow-hidden block"
        ...
        <div className="max-w-4xl w-full mx-auto relative z-20 px-4">
          <div className="glass-dark p-2 rounded-3xl w-full flex flex-col md:flex-row items-stretch md:items-center gap-2 shadow-2xl relative z-10 animate-scaleIn select-none border border-white/10"
            ...
            <div className="flex-[2] min-w-0 flex items-center px-4 bg-white/5 md:bg-transparent rounded-2xl md:rounded-none border border-white/5 md:border-none">
              ...
              <input type="text" value={query} onChange={e => setQuery(e.target.value)}
                placeholder={`الوجهة: ${typed}`} dir="rtl"
            ...
            {/* DATE RANGE PICKER */}
            <div className="relative flex-1 min-w-0 flex items-center px-4 py-2 bg-white/5 md:bg-transparent rounded-2xl md:rounded-none group cursor-pointer border border-white/5 md:border-none"
              ...
              {mounted && showCalendar && (
                <div className="absolute top-full left-1/2 -translate-x-1/2 md:translate-x-0 md:left-auto md:right-0 mt-3 z-[200] animate-scaleIn" onClick={e => e.stopPropagation()}>
                  <Calendar
                    onChange={(val: any) => {
                      setDateRange(val);
                    ...
                    locale="ar-EG"
                  />
                  <button onClick={() => { setDateRange(null); setShowCalendar(false); }}
            ...
            {/* BUTTON */}
            <a href={s.href} className="md:ml-2">
              <button className="px-6 py-4 md:py-3 rounded-2xl w-full md:min-w-[150px] text-sm font-black text-white btn-shine transition-all hover:scale-[1.03]"
          ...
          <div className="flex flex-wrap justify-center gap-2 mt-3">
            {["شرم الشيخ", "الغردقة", "العمرة", "الأقصر", "أسوان", "الحج"].map(t => (
        ...
        {/* Slide dots */}
        <div className="hidden md:flex absolute bottom-16 md:bottom-24 flex items-center gap-2.5">
          {SLIDES.map((_, i) => (
